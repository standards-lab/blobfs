...
// every order is total. A directory that does not exist lists no rows and
// a total of zero.
...
// page carries a Next only when More is true and its sort can be
// continued: the terms up to the name tie-breaker run in one direction
// and name no nullable field, which here is size or etag, null until a
// write completes. Any other sort pages by number only. A cursor the
// library did not issue, one edited, one issued by the directory listing
// or under other filters or another sort, and any cursor under a sort
// that cannot be continued are refused with a query.CursorError before
// any SQL. A cursor is a position in the order, not a bookmark on the
// directory: the library does not record directoryID in it.
