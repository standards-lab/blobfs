...
-- ErrNotEmpty; there is no cascade, and no version guard, since the keys
-- refuse the one case a stale version would catch. A consumer's foreign key
-- into blobfs_directory refuses the removal as ErrReferenced. No row
-- affected means the directory does not exist. One statement, so it
-- accepts the pool and composes into a caller's transaction alike.
